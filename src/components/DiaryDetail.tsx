...
export function DiaryDetail({ id }: { id: string }) {
  ...
  return <div className="page-scroll detail-page"><div className="detail-actions detail-actions-top"><button className="button button-ghost danger-border detail-delete-button" onClick={() => setConfirmOpen(true)} disabled={busy} aria-label="删除这一页" title="删除这一页"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M10 11v6m4-6v6M9 7l1-2h4l1 2m-9 0 1 13h10l1-13" /></svg></button><button className="button button-primary" onClick={() => void togglePublish()} disabled={busy}>{entry.publishedAt ? "取消分享" : "分享"}</button></div><article className="saved-card"><div className="saved-cover">{cover ? <ImagePreview src={cover} alt="音乐日记封面" className="image-preview-fill"><Image src={cover} alt="音乐日记封面" fill sizes="(max-width: 700px) 310px, 230px" unoptimized /></ImagePreview> : <div className="cover-placeholder">Meloday</div>}</div><div className="saved-main"><span className="eyebrow">{formatDate(entry.createdAt)}</span><h1>{entry.title}</h1><p className="saved-summary">{entry.summary}</p>{audio && <AudioPlayer src={audio} />}</div></article><ConfirmDialog open={confirmOpen} title="确定删除这一页吗？" description="" confirmLabel="删除这一页" busy={busy} onCancel={() => setConfirmOpen(false)} onConfirm={() => void remove()} /></div>;
}
